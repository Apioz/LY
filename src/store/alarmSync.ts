...
/** 小程序设施工单展示状态 */
export const MINI_FACILITY_STATUS = ['待派单', '待接单', '处理中', '待完成', '已完成', '已取消', '损坏'] as const
...
const initialFacility: FacilityOrderItem[] = [
  ...
  },
  {
    ...
    flowRecords: [
      ...
      { time: '2026-05-28 16:30:00', action: '维修人员接单', operator: '李维修', fields: [{ label: '工单状态', value: '处理中' }] },
      {
  ...
  },
  {
    ...
    flowRecords: [
      ...
      { time: '2026-05-20 09:00:00', action: '维修人员接单', operator: '张工', fields: [{ label: '工单状态', value: '处理中' }] },
      {
...
export function startFacilityRepair(orderId: string, receiver: string) {
  facilityOrders = facilityOrders.map((o) => {
    if (o.id !== orderId || o.miniStatus !== '处理中' || o.receiver !== receiver || o.repairStarted) return o
    return {
      ...o,
      repairStarted: true,
...
function canEditRepairOrder(o: FacilityOrderItem, receiver: string) {
  return (
    (o.miniStatus === '处理中' || o.miniStatus === '待完成') &&
    o.receiver === receiver &&
    !!o.repairStarted
  )
}
...
export function saveFacilityRepairDraft(orderId: string, receiver: string, payload: FacilitySubmitPayload) {
  facilityOrders = facilityOrders.map((o) => {
    if (o.id !== orderId || !canEditRepairOrder(o, receiver) || o.miniStatus !== '处理中') return o
